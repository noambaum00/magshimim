...
typedef struct bingoPlayer //no typdef
{
	char name[STR_LEN]; //not place in the struct
...
void freeMemory(bingoGame game);
...
int main(void)
{
	...
	freeMemory(game); //function  missing - memory leak
	getchar();
	return 0;
}
...
void freeMemory(bingoGame game)
{
	free(game.numsDrawn); //function  missing - memory leak
	free(game.players);
}
